...
```text
Excel -> 结构化 IR -> 校验 -> Verilog 生成 -> 图形预览
```
...
3. `yaml_io.py`：YAML 读写。
4. `validator.py`：基础校验。
5. `verilog_emit.py`：Verilog 生成。
6. `graph_emit.py`：Mermaid/Graphviz 生成。
...
| M1 | 完成文档与数据模型样例 |
| M2 | 完成 Excel -> YAML 导入 |
| M3 | 完成 YAML -> Verilog 生成 |
| M4 | 完成校验器和错误报告 |
| M5 | 完成 Mermaid/Graphviz 图生成 |
| M6 | 完成 React Flow Demo |
...
4. 实现 Python CLI 的 `validate` 和 `generate-verilog`。
5. 用一个三模块示例验证完整流程。
